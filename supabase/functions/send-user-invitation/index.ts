...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    await checkRateLimit('send-user-invitation', identifier, 20, 60);
    ...
    const { email, role, tenantName, inviterName, temporaryPassword } = await parseBodyWithLimit(req) as unknown as InvitationRequest;

    console.log('Sending user invitation to:', email);

    const roleNames: Record<string, string> = {
      instructor: '강사', student: '학생', admin: '관리자',
    };
      ...
      body: JSON.stringify({
        ...
        html: `<div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="margin: 30px 0;">
            <a href="https://yourdomain.com/auth" style="background-color: #4CAF50; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block;">로그인하기</a>
          </div>
        </div>`,
      }),
    });
