...
const DashboardLayout = ({ children, userRole, isDemo = false }: DashboardLayoutProps) => {
  ...
  const getDefaultMenuItems = (): MenuItem[] => {
    ...
    const adminItems = [
      ...
      { icon: DollarSign, label: "매출 관리", path: "/admin/revenue", enabled: true },
        ...
        { icon: Palette, label: "홈페이지 관리", path: "/operator/homepage-settings", enabled: true },
  ...
  // 🚀 PERFORMANCE: HRD 설정을 React Query로 캐싱
      ...
      {!isDemoMode && (
        ...
        <div className="flex h-16 md:h-20 items-center gap-2 md:gap-4 px-3 md:px-6">
          ...
          <Link 
            ...
            className="flex items-center gap-1.5 md:gap-2 group flex-shrink-0 hover:opacity-80 transition-opacity relative z-50 cursor-pointer"
          >
            <img src={logoIcon} alt="Logo" className="h-9 w-9 md:h-12 md:w-12 pointer-events-none" />
            <span className="text-lg md:text-2xl font-logo font-bold text-foreground tracking-tight pointer-events-none">atomLMS</span>
          </Link>
